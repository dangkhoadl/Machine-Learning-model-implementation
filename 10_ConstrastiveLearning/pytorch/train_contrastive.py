...
from src.datasets import Pretrained_Dset, Downstream_Dset, unpickle, lab_dict
from src.models import PretrainedModel, DownstreamModel
from src.trainers import PretrainTrainer, DownstreamTrainer
...
if __name__ == "__main__":
    ...
    def pretrain():
        ...
        # LOSS FUNCTION
        criterion = hparams['pretrain_criterion']
        ...
        trainer = PretrainTrainer(
            ...
            exp_name=hparams['exp_name'], device=device, mode='pretrain')

        trainer.train(
            num_epochs=hparams['pretrain_epochs'],
            eval_every_epochs=hparams['pretrain_eval_every_epochs'],
            resume_from_chkpt=hparams['pretrain_resume'],
            plot_feats=True)
    pretrain()
    ...
    def train_downstream():
        ...
        premodel.load_state_dict(torch.load(
            f"exp/{hparams['exp_name']}/chkpts/best_pretrained.pt")['model'])
        ...
        trainer.train(
            num_epochs=hparams['downstream_epochs'],
            eval_every_epochs=5,
