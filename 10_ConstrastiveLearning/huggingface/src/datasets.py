    ...
    def __getitem__(self, idx):
        ...
        return {
            'input_values': torch.stack([
                x.squeeze(dim=0),
                x1.squeeze(dim=0),
                x2.squeeze(dim=0)], dim=0).to(torch.float32),
            'labels': torch.tensor(self.label_arr[idx], dtype=torch.long),}
...
class Downstream_Dset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Augment
        if self.mode == 'train':
            x_aug  = self.randomcrop(x)

        # Normalize
        x = self.__normalize(x)
        if self.mode == 'train':
            x_aug = self.__normalize(x_aug)
        ...
        if self.mode == 'train':
            return {
                'input_values': torch.stack([
                    x.squeeze(dim=0),
                    x_aug.squeeze(dim=0)], dim=0).to(torch.float32),
                'labels': torch.tensor(self.label_arr[idx], dtype=torch.long),
            }
        else:
            return {
                'input_values': x.to(torch.float32),
                'labels': torch.tensor(self.label_arr[idx], dtype=torch.long),
            }
